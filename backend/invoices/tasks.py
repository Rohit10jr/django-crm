...
def send_email(invoice_id, recipients, domain="demo.django-crm.io", protocol="http"):
    ...
    # OPTIMIZED: Fetch all active users in ONE query
    # active_users = User.objects.filter(id__in=recipients, is_active=True)
    # for user in active_users:
    # [!!] query inside loops is not sound choice because it will hit the database multiple times.
    for user in recipients:
        ...
        if user:
            ...
            context["url"] = (
                ...
                # [??] Check this. Invoice URL namespace is, app_name=api_invoices
            )
            ...
            msg.send()
    # [??]  check the logic of this filter
...
# [!!] check if we can combine send_invoice_email and send_invoice_email_cancel into one function with an extra parameter
# [!!] try to use request.build_absolute_uri() or set BASE_URL in settings
@app.task
...
def create_invoice_history(original_invoice_id, updated_by_user_id, changed_fields):
    ...
    # [!!] add this condition above so that we can skip computing when dont have to create history for created invoice
