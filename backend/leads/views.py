        ...
        if serializer.is_valid():
            ...
            if data.get("tags", None):
                ...
                for t in tags:
                    ...
                    # tag, _ = Tags.objects.get_or_create(slug=t.lower(), defaults={"name": t})
                    if tag.exists():
                        # [!!] use first in filter instead of this
                        tag = tag[0]
                    else:
            ...
            if data.get("status") == "converted":
                ...
                # [!!] after assigning to objects we need account_object.save()
...
class LeadDetailView(APIView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # [!!] better option -> 
        # user_assgn_list = list(self.lead_obj.assigned_to.values_list('id', flat=True))
        # [!!] and why add user object when user_assgn_list contains only id's
        if self.request.profile.user == self.lead_obj.created_by:
            user_assgn_list.append(self.request.profile.user)
        if self.request.profile.role != "ADMIN" and not self.request.user.is_superuser:
            # [!!] code adds user object and now checks for id, wrong logic
        ...
        # [!!] for nested dict i think we can use values()
        assigned_data = []
    ...
    def post(self, request, pk, **kwargs):
        ...
        # self.lead_obj = get_object_or_404(
        #     Lead, pk=pk, org=request.profile.org
        # )
    ...
    def put(self, request, pk, **kwargs):
        ...
        if serializer.is_valid():
            ...
            lead_obj.tags.clear()
            # [!!] replacement for below condition
            # tag, _ = Tags.objects.get_or_create(slug=t.lower(), defaults={"name": t})
            if params.get("tags"):
            ...
            # [!!] place this logic after the if params.get("assigned_to"): condition
            ...
            # [!!] use this condition if "tags" in params:
            if params.get("contacts"):
                obj_contact = Contact.objects.filter(
                    id=params.get("contacts"), org=request.profile.org
                )
                # [!!] * is missing *obj_contact)
                lead_obj.contacts.add(obj_contact)
...
class CompanyDetail(APIView):
    ...
    permission_classes = (IsAuthenticated,)
    # [!!] no validation to check if company is realted to user
    # test this in this view
    # request.data["org"] = request.profile.org.id
    def get_object(self, pk):
        try:
            return Company.objects.get(pk=pk)
        except Company.DoesNotExist:
            # [??] Http404 import was missing
            raise Http404
