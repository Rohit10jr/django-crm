...
class MagicLinkRequestView(APIView):
    ...
    def post(self, request):
        ...
        # Dev convenience: print the clean sign-in link / code to the runserver
        # terminal so it can be copied without quoted-printable email artifacts.
        if settings.DEBUG:
            if delivery == "code":
                print(
                    f"\n{'=' * 72}\n  DEV OTP CODE ({email}): {raw_code}\n{'=' * 72}\n",
                    flush=True,
                )
            else:
                _dev_url = f"{settings.FRONTEND_URL}/login/verify?token={token_obj.token}"
                print(
