...
class CaseListView(APIView, LimitOffsetPagination):
    ...
    # [??] use get_queryset instead of get_context_data and move filter logic to get_queryset
    def get_context_data(self, **kwargs):
        ...
        if params:
            # [!!] we can optimize this by creating a dict of filter params and passing it to filter method instead of multiple if conditions
            # [??] check how we can implement django-filter here
            if params.get("name"):
...
class CaseDetailView(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        # [!!] redundant logic, org filter is applied in get_object 
    ...
    # [??] pk is to identify case
    def post(self, request, pk, **kwargs):
        params = request.data
        # [??] use get_object here instead of direct query and remove redundant org filter in get_object
        ...
        if comment_serializer.is_valid():
            # [!!] serializer.is_valid should be enough to check if comment is present or not, remove redundant check
