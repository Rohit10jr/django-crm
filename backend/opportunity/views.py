...
# [!!] use @transaction.atomic for model creation and trigger celery task
# class OpportunityListView(ListCreateAPIView):
#     serializer_class = OpportunitySerializer
#     permission_classes = [IsAuthenticated]
#     pagination_class = LimitOffsetPagination
    # def get_queryset(self):
    #     qs = Opportunity.objects.filter(org=self.request.profile.org)
    ...
    # move create logic in modelserializer

class OpportunityListView(APIView, LimitOffsetPagination):

    # authentication_classes = (CustomDualAuthentication,)
    permission_classes = (IsAuthenticated,)
    model = Opportunity # [!!] APIView does not have model attribute by default

    # [!!] use get_opportunity_list_data as method name
    def get_context_data(self, **kwargs):
        ...
        if params:
            ...
            # __in expects a list, should be params.getlist("tags") 
            if params.get("tags"):
                queryset = queryset.filter(tags__in=params.get("tags")).distinct()

        context = {}
        # [!!] why not use drf get_paginated_response instead of manually paginating and creating response
    ...
    # [!!] maybe create service/OpportunityService to handle business logic and call it from views.
    @extend_schema(
        tags=["Opportunities"],
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if self.request.FILES.get("opportunity_attachment"):
                ...
                # [!!] Why created_by is set to user instead of profile, 
                # [!!] also both these fields are missing in attachments and base model
...
class OpportunityDetailView(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        # [!!] use context to send the request
...
class OpportunityCommentView(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        if (
            ...
            # [!!] the params logic is redundant as validation checks if the comment field exists or not in serializer
            if params.get("comment"):
                if serializer.is_valid():
                    serializer.save()
                    return Response(
